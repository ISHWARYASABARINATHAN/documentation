...
To change the view, so only the entries for a single day, week, month, quarter, or year are shown,
click the :icon:`fa-calendar` :guilabel:`(Month)(Year)` button. A drop-down menu appears with the
options of :guilabel:`Today`, :guilabel:`This week`, :guilabel:`This month`, :guilabel:`This
...
are any errors on the work entry, such as required fields being left blank. Conflicts that span the
current pay period being processed **must** be resolved before payslips can be generated.
...
The time off conflict can be resolved either on the :ref:`work entry itself,
<payroll/time-off-work-entry>` or on a detailed :ref:`time off request form
<payroll/time-off-request-form>`.
...
.. note::
   If the itme off is approved, the status of the work entry conflict changes to
